...
| REQ-015 *(Rev 5)* | The system shall provide open-loop-capable PWM/speed-setpoint control of 3 independently-driven reaction wheels, one per orthogonal axis (X/Y/Z), extending REQ-007's single-axis capability to all 3 axes | Must | Final electromechanical scale-out step of the roadmap (`docs/architecture.md` §11). Per-axis torque/RPM/momentum targets are proposed **unchanged** from Rev 3's already-approved figures — **§9h Q1 CONFIRMED via creator/"General Chat" session (§9i), conditional on Q2 (REQ-021) staying "deferred"**; if Q2 ever flips to "jump maneuver in scope," this figure needs a fresh impulsive-torque derivation, not steady-state reuse |
...
| REQ-020 *(Rev 5)* | The system should provide a wireless remote-control link (UART-bridge module or MCU-native radio) for commanding maneuvers and monitoring status without a physical tether | Should | Proposed **Should**, not Must — **§9h Q4 CONFIRMED via creator/"General Chat" session (§9i)**. Deliberately reverses REQ-006's "no wireless functionality actually implemented even if silicon has it" scope fence for this revision specifically — a recorded, deliberate reversal, not a silent contradiction of it |
| REQ-021 *(Rev 5)* | The Cubli-style "spin up 1+ wheels, then hard-brake to transfer momentum and jump the assembly onto an edge/vertex, then balance" maneuver is **proposed Won't (this cycle) — NOT YET FINALIZED** | Won't (this cycle), **proposed/working default only — pending human Chief Engineer confirmation, see §9i** | **§9h Q2 — explicitly NOT decided by the creator session; reserved for the human Chief Engineer directly (§9i).** Rev 5's own scope is provisionally the 3-axis attitude-hold/momentum-management hardware + control capability, deferring the jump-and-balance maneuver as a materially harder, later capability — mirroring this project's own "favor the faster/leaner path" precedent (§9f) — **but this cannot be assumed to be what the human wants**: he has reportedly described this whole revision as "what I want to build is the JAXA one," whose signature demonstration *is* this maneuver. Used as a working assumption only, so Requirements-phase work isn't blocked. **Binding until the human answers**: no irreversible downstream choice may foreclose a future "yes" (Power Engineer must not rule out battery power, REQ-115; brake sizing must not assume steady-state-only torque) — see §9i for the full record and REQ-009/014's own pattern this otherwise mirrors |
...
| REQ-115 *(Rev 5)* | The physical power source architecture (Rev 3's approved tethered Option A: USB 5V logic + barrel-jack ~12V motor rail, `hardware/power-architecture.md`) shall be **re-evaluated**, not assumed to extend unchanged, given the new 3-motor+3-brake simultaneous load and the roadmap's eventual untethered "stand on a vertex" maneuver | Must | **New finding this session, §9h Q4 CONFIRMED via creator/"General Chat" session (§9i)**: a barrel-jack tether is likely physically incompatible with any untethered maneuver; battery (Rev 3's own explicitly-rejected Option C) must be kept a **live, not re-foreclosed** option, precisely because REQ-021/Q2 (the jump maneuver) is not yet settled by the human — Power Engineer's future pass must not commit to "tethered-only forever" while that remains open. Not decided here — routed to a fresh Power Engineer HITL gate once real multi-channel numbers exist |
...
| REQ-409 *(Rev 5, new — not an edit to REQ-403)* | The aggregate stored rotational energy across 3 simultaneous reaction wheels, plus the new hazard of hard electromagnetic braking (rapid momentum transfer, potentially violent for a desaturation or future jump maneuver), shall be assessed and mitigated as its own safety analysis before this revision's Design-Complete-equivalent sign-off | Must | Safety-critical — ties to the "safety-critical changes" HITL gate (`docs/architecture.md` §10), same gate class as REQ-403/407. **Explicitly does not reopen or edit REQ-403/MISS-016** (`validation/open-issues.md`, single-flywheel containment, human `ACCEPTED-RISK` 2026-09-13) — that disposition covered exactly one flywheel at Rev 3/4's speeds and is left untouched. REQ-409 is the fresh requirement that actually governs the 3-flywheel + braking hazard shape going forward. Flagged for the Mechanical Reviewer's existing Foresight checklist (`docs/architecture.md` §3, `docs/architecture-evolution.md` §38) with **extra rigor**, once a real mechanical design exists to review — not resolved here. **§9h Q5 CONFIRMED via creator/"General Chat" session (§9i)**: framing pitched correctly, neither over- nor under-claiming |
...
| REQ-507 *(Rev 5)* | Rev 5 total BOM cost target: **CONFIRMED** ≤ $450–800+ USD (soft, not a hard ceiling) | Should | Reflects the cost/complexity trade-off already presented to and approved by the human Chief Engineer as part of this scope decision (see header status line, §1c) — a 5–9× jump over Rev 3/4's ≈$75–90 ceiling, driven by 3× maxon-or-equivalent-class motors, 6× IMUs, 3× electromagnetic brakes, and a likely MCU upgrade. **§9h Q3 CONFIRMED via creator/"General Chat" session (§9i)** |
...
- **Q2 (Cubli-style jump-and-balance maneuver in/out of scope, REQ-021) —
  ...
  The human's own words when scoping this whole revision were reported
  as **"what I want to build is the JAXA one"** — which means the
  conservative deferral (REQ-021's own proposed default) cannot be safely
  assumed to be what he wants just because it is the lower-risk
  engineering choice. **REQ-021 therefore remains an explicitly proposed
  working default, not a finalized requirement** — used so downstream
