...
| REQ-020 *(Rev 5)* | The system should provide a wireless remote-control link (UART-bridge module or MCU-native radio) for commanding maneuvers and monitoring status without a physical tether | Should | Proposed **Should**, not Must — open question, §9h Q4. Deliberately reverses REQ-006's "no wireless functionality actually implemented even if silicon has it" scope fence for this revision specifically — a recorded, deliberate reversal, not a silent contradiction of it |
| REQ-021 *(Rev 5)* | The Cubli-style "spin up 1+ wheels, then hard-brake to transfer momentum and jump the assembly onto an edge/vertex, then balance" maneuver is **Won't (this cycle)** | Won't (this cycle) | Proposed default — §9h Q2. Rev 5's own scope is the 3-axis attitude-hold/momentum-management hardware + control capability; the jump-and-balance maneuver is a materially harder, later capability (its own torque/timing/structural-impact requirements) deliberately deferred rather than front-loaded, mirroring this project's own repeated "favor the faster/leaner path to a real result" precedent (§9f). Explicit anti-scope statement, mirrors REQ-009/014's own pattern |
...
| REQ-115 *(Rev 5)* | The physical power source architecture (Rev 3's approved tethered Option A: USB 5V logic + barrel-jack ~12V motor rail, `hardware/power-architecture.md`) shall be **re-evaluated**, not assumed to extend unchanged, given the new 3-motor+3-brake simultaneous load and the roadmap's eventual untethered "stand on a vertex" maneuver | Must | **New finding this session, flagged open (§9h Q4)**: a barrel-jack tether is likely physically incompatible with any untethered maneuver; battery (Rev 3's own explicitly-rejected Option C) may need to be reconsidered. Not decided here — routed to a fresh Power Engineer HITL gate once real multi-channel numbers exist |
...
| REQ-409 *(Rev 5, new — not an edit to REQ-403)* | The aggregate stored rotational energy across 3 simultaneous reaction wheels, plus the new hazard of hard electromagnetic braking (rapid momentum transfer, potentially violent for a desaturation or future jump maneuver), shall be assessed and mitigated as its own safety analysis before this revision's Design-Complete-equivalent sign-off | Must | Safety-critical — ties to the "safety-critical changes" HITL gate (`docs/architecture.md` §10), same gate class as REQ-403/407. **Explicitly does not reopen or edit REQ-403/MISS-016** (`validation/open-issues.md`, single-flywheel containment, human `ACCEPTED-RISK` 2026-09-13) — that disposition covered exactly one flywheel at Rev 3/4's speeds and is left untouched. REQ-409 is the fresh requirement that actually governs the 3-flywheel + braking hazard shape going forward. Flagged for the Mechanical Reviewer's existing Foresight checklist (`docs/architecture.md` §3, `docs/architecture-evolution.md` §38) with **extra rigor**, once a real mechanical design exists to review — not resolved here |
...
| REQ-507 *(Rev 5)* | Rev 5 total BOM cost target: proposed ≤ $450–800+ USD (soft, not a hard ceiling) | Should | Reflects the cost/complexity trade-off already presented to and approved by the human Chief Engineer as part of this scope decision (see header status line, §1c) — a 5–9× jump over Rev 3/4's ≈$75–90 ceiling, driven by 3× maxon-or-equivalent-class motors, 6× IMUs, 3× electromagnetic brakes, and a likely MCU upgrade. Confirm this exact figure — §9h Q3 |
