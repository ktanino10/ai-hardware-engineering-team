...
const entryPath = path.resolve(root, options.entry || 'visualization/rev5-full-assembly-v2/index.html');
...
const docs = path.resolve(root, options['evidence-dir'] || '.agent-work/rev5-browser-evidence');
...
const evidence = { schema_version: 1, started_at: new Date().toISOString(),
  ...
  mode: `isolated headless Chrome; ${entryURL.protocol}; no installed packages or shared browser profile`,
  checks: [], exceptions: [], console_errors: [], loader_errors: [], external_requests: [], screenshots: [], status: 'RUNNING' };
let socket, sequence = 0;
const pending = new Map();
function send(method, params = {}) {
  const id = ++sequence;
  return new Promise((resolve, reject) => {
    const timeout = setTimeout(() => { pending.delete(id); reject(new Error(`CDP timeout: ${method} ${(params.expression || '').slice(0, 160)}`)); }, 20000);
...
try {
  ...
  for (const name of ['index.html', 'style.css', 'interactive-viewer.js', 'scene-data.js', 'edge-data.js', 'features.js', 'viewer.js', 'drawings.js', 'app.js', 'inventory.json', 'runtime-assets.json', 'README.md']) {
  ...
  socket.addEventListener('message', event => {
    ...
    if (message.id) {
    ...
    else if (message.method === 'Network.loadingFailed') evidence.loader_errors.push(message.params);
      ...
      if (!options.url || requested.origin !== entryURL.origin ||
          !requested.pathname.startsWith(new URL('.', entryURL).pathname)) evidence.external_requests.push(requested.href);
    }
  ...
  for (const [amount, camera] of [[0, 'iso'], [100, 'side']]) {
    await settle(`document.getElementById('part-select').value='X_motor';document.getElementById('part-select').dispatchEvent(new Event('change'));document.getElementById('isolate').click();document.getElementById('explode').value='${amount}';document.getElementById('explode').dispatchEvent(new Event('input'));document.querySelector('[data-camera=${camera}]').click()`);
    ...
    evidence.checks.push({ name: 'actual pointer hover and click pin at current exploded triangle geometry', amount, camera, tooltip });
  }
  ...
  await send('Browser.setDownloadBehavior', { behavior: 'allow', downloadPath: scratch });
  await settle("document.getElementById('save-sheet').click()");
  let saved;
  for (let i = 0; i < 40; i++) {
    try { saved = await readFile(path.join(scratch, 'rev5-X_motor-REF.svg'), 'utf8'); break; }
    catch (error) { if (error.code !== 'ENOENT') throw error; await sleep(100); }
  }
  ...
  if (chrome.pid && chrome.exitCode === null) chrome.kill('SIGTERM');
  if (chrome.pid) {
  ...
  evidence.owned_browser = { exit_code: chrome.exitCode, signal: chrome.signalCode, stopped: !chrome.pid || chrome.exitCode !== null || chrome.signalCode !== null };
