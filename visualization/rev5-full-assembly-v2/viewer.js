(function (root) {
  ...
  const cameras = { iso: [1, -1.4, 1], front: [0, -1, 0], side: [1, 0, 0], top: [0, 0, 1] };
  const colors = {
  ...
  class WholeViewer {
    constructor(canvas, scene, meshes, edges, onState, onError) {
      ...
      this.state = { ...math.initialState(cameras.iso), selected: 'X_motor', isolated: false, dimensions: true,
        phase: 0, playing: false, appearance: 'opaque',
        groups: Object.fromEntries(Object.keys(scene.groups).map(key => [key, !['panels', 'reference'].includes(key)])) };
    ...
    offset(component) { return root.Rev5Features.offset(component, this.state.explode); }
    camera() {
      return math.viewProjection(this.state, { center: [0, 0, 0], radius: 205 + this.state.explode * 190 }, this.canvas.width / this.canvas.height);
    }
    preset(name) {
      failIf(!cameras[name], '不明な視点です。');
      this.setState({ ...math.cameraPreset(this.state, cameras[name]), autoOrbit: false });
    }
    visible(component) { return this.state.isolated ? component.id === this.state.selected : this.state.groups[component.group]; }
    getState() {
    ...
    drawDimensions() {
      ...
      if (this.state.dimensions && this.state.explode === 0) {
      ...
      for (let axis = 0; axis < 3; axis++) {
        ...
        for (let side = 0; side < 4; side++) {
          ...
          line(a, b, side === 0 ? `${'XYZ'[axis]} ${c.dimensions_mm[axis].toFixed(2)}` : '', 'selection-line');
        }
      }
    }
  }
  root.Rev5Whole = { unpack, WholeViewer, cameras };
})(globalThis);
