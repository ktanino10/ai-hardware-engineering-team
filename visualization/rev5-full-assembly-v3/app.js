(async function () {
  ...
  try {
    ...
    const dependencies = {
      ...
      REV5_PLAN_SOURCE: 'plan-data.js',
      REV5_MEDIA_SOURCE: 'media-data.js',
      Rev5Media: 'media.js',
    };
    ...
    const plan = await Rev5Features.loadPlan(REV5_PLAN_SOURCE, scene);
    const mediaData = await Rev5Media.load(REV5_MEDIA_SOURCE, plan, REV5_PLAN_SOURCE.sha256);
    const media = Rev5Media.mount(mediaData, plan);
    for (const unit of plan.named_units) {
      const option = document.createElement('option'); option.value = unit.id;
      option.textContent = `${unit.name} (${unit.members.length})`; byId('unit-select').append(option);
    }
    plan.stages.forEach((stage, i) => {
      const button = document.createElement('button'); button.dataset.stage = String(i);
      button.textContent = `${i+1} ${stage.name}`; button.disabled = true; byId('stage-controls').append(button);
    });
    byId('plan-source').textContent = `共通表示プラン SHA-256 ${REV5_PLAN_SOURCE.sha256}`;
    ...
    const update = state => {
      ...
      byId('view-heading').textContent = state.isolated ? Rev5Features.name(component) :
        state.focusUnit ? plan.named_units.find(u => u.id === state.focusUnit).name : '全体の配置';
      byId('visible-count').textContent = `${state.visibleIds.length} / ${scene.component_count} 形状を表示${state.frameBounds.empty ? ' · 表示対象なし' : ''}`;
      ...
      const unit = plan.named_units.find(u => u.id === state.detailUnit);
      byId('unit-select').value = state.detailUnit;
      byId('stage-status').textContent = state.isolated ? '単体メッシュ：分割なし。ターンテーブルで多方向を見る。' :
        `${Rev5Features.stage(state.explode)} · 最終段階の対象：${unit.name}`;
      byId('unit-status').textContent = `${state.focusUnit ? 'ユニット中心の視点' : state.isolated ? '選択形状中心の視点' : '全体の視点'} / ${unit.detail_explodable ? '表現済みの構成形状のみ段階表示' : '追加の分割なし・多方向表示のみ'} / 関連付けは表示用推定`;
      byId('play').disabled = state.reducedMotion || state.isolated || state.focusUnit === 'reference-only';
      byId('orbit').disabled = state.reducedMotion;
      byId('explode').disabled = state.isolated || state.focusUnit === 'reference-only';
      document.querySelectorAll('[data-stage]').forEach(button => {
        button.disabled = state.isolated || state.focusUnit === 'reference-only';
        button.setAttribute('aria-pressed', String(state.explode > plan.stages[Number(button.dataset.stage)].range[0] &&
          state.explode <= plan.stages[Number(button.dataset.stage)].range[1]));
      });
      ...
      byId('exterior').setAttribute('aria-pressed', String(!state.focusUnit && !state.isolated && state.groups.panels && state.groups.frame));
      byId('interior').setAttribute('aria-pressed', String(!state.focusUnit && !state.isolated && !state.groups.panels && state.groups.frame));
      byId('drive-only').setAttribute('aria-pressed', String(!state.focusUnit && !state.isolated && state.groups.drive && Object.entries(state.groups).every(([k, v]) => k === 'drive' || !v)));
      media.updateSelection(component.id);
    ...
    for (const [key, input] of Object.entries(checks)) input.addEventListener('change', () => patch({ isolated: false, focusUnit: null, zoom: 1, groups: { ...viewer.state.groups, [key]: input.checked } }));
    const mode = name => patch({
      isolated: false, focusUnit: null, zoom: 1,
      groups: Object.fromEntries(Object.keys(scene.groups).map(key => [key, plan.whole_modes[name].groups.includes(key)])),
    });
    ...
    byId('isolate').addEventListener('click', () => patch({ isolated: true, focusUnit: null, zoom: 1, explode: 0, phase: 0, playing: false, autoOrbit: false }));
    byId('unit-select').addEventListener('change', event => {
      const detailUnit = event.target.value;
      const unit = plan.named_units.find(u => u.id === detailUnit);
      const selected = unit.members.includes(viewer.state.selected) ? viewer.state.selected :
        unit.members.find(id => id.endsWith('_motor')) || unit.members[0];
      patch({ detailUnit, focusUnit: viewer.state.focusUnit ? detailUnit : null,
        selected: viewer.state.focusUnit ? selected : viewer.state.selected, playing: false, autoOrbit: false, zoom: 1 });
    });
    byId('focus-unit').addEventListener('click', () => {
      const unit = plan.named_units.find(u => u.id === viewer.state.detailUnit);
      const selected = unit.members.includes(viewer.state.selected) ? viewer.state.selected :
        unit.members.find(id => id.endsWith('_motor')) || unit.members[0];
      patch({ focusUnit: unit.id, isolated: false, selected, zoom: 1, playing: false, autoOrbit: false });
    });
    byId('full-view').addEventListener('click', () => mode('exterior'));
    document.querySelectorAll('[data-stage]').forEach(button => button.addEventListener('click', () => {
      const explode = plan.stages[Number(button.dataset.stage)].range[1];
      patch({ explode, phase: explode, playing: false, autoOrbit: false });
    }));
    ...
    byId('play').addEventListener('click', () => { if (!viewer.reducedMotion.matches && !viewer.state.isolated && viewer.state.focusUnit !== 'reference-only') patch({ playing: true }); });
