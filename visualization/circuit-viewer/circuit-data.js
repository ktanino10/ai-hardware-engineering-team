...
const WIRES = [
  ...
  { from:'J1', to:'U4', fromSide:'right', toSide:'left', category:'power5v', net:'VBUS_5V', dir:1,
    why:'Raw 5V straight off the USB-C cable\'s power pins, routed through ESD protection (U4) before it reaches anything else — a cable transient gets clamped here, not at the regulator.' },
  { from:'J1', to:'U3', fromSide:'right', toSide:'top',  category:'power5v', net:'VBUS_5V', dir:1,
    why:'Same raw USB 5V feeding the LDO\'s IN pin. This is the only power source on the whole board — there is no battery and no other input.' },
  { from:'U3', to:'U1', fromSide:'right', toSide:'left', category:'power5v', net:'/3V3', dir:1, yoff:-15,
    why:'Regulated 3.3V logic rail. Chosen because the STM32G031 MCU is a 3.3V-class part — feeding it 5V directly would exceed its absolute maximum rating and likely destroy it.' },
  { from:'U3', to:'U2', fromSide:'right', toSide:'left', category:'power5v', net:'/3V3', dir:1, yoff:10,
    why:'The same 3.3V rail, also supplying the IMU\'s VDD/VDDIO pins. C6-C9 decouple it locally right at the IMU so digital switching noise elsewhere on the board doesn\'t corrupt sensor readings.' },

  // ---- power: external motor rail ----
  { from:'J4', to:'F1', fromSide:'right', toSide:'left', category:'powervm', net:'VM_MOTOR_RAW', dir:1,
    why:'Unregulated DC straight from the external barrel-jack supply — whatever voltage a bench power supply is set to. Nothing on this board regulates it down before the motor driver.' },
  { from:'F1', to:'D2', fromSide:'right', toSide:'left', category:'powervm', net:'VM_MOTOR_F1', dir:1,
    why:'Passed through F1 first — a resettable PTC fuse. If the motor rail ever pulls too much current, F1\'s resistance rises and throttles it, then it resets itself once the fault clears (nothing to replace).' },
  ...
  { from:'U6', to:'U5', fromSide:'right', toSide:'left', category:'powervm', net:'U5_VCC', dir:1,
    why:'Only present at all once U6 has been told to arm the rail. Before that this net sits at 0V and the motor driver is fully unpowered — a deliberate safety interlock, not a wiring gap.' },
  { from:'U5', to:'M1', fromSide:'right', toSide:'left', category:'motorphase', net:'MOTOR_PHASE_U', dir:1, yoff:-18,
    why:'One of three 120°-phased AC waveforms U5 synthesizes from the flat DC on U5_VCC to spin the motor. This is real motor current, not a logic signal — its frequency sets the RPM.' },
  { from:'U5', to:'M1', fromSide:'right', toSide:'left', category:'motorphase', net:'MOTOR_PHASE_V', dir:1, yoff:0,
    why:'Second of the three motor phases — 120° out of phase with U and W. All three together make the rotating field that spins the BLDC rotor.' },
  { from:'U5', to:'M1', fromSide:'right', toSide:'left', category:'motorphase', net:'MOTOR_PHASE_W', dir:1, yoff:18,
    why:'Third motor phase. U5 is "sensorless" — it infers rotor position from the back-EMF on these same three wires rather than needing separate position sensors.' },
  { from:'M1', to:'MOTOR', fromSide:'right', toSide:'left', category:'motorphase', net:'phase wires', dir:1,
    why:'The same three phase wires continuing off-board, through this terminal block, to the actual motor windings.' },
  ...
  { from:'U2', to:'U1', fromSide:'left', toSide:'right', category:'i2c', net:'I2C2_SCL/SDA', dir:1, yoff:-20,
    why:'The MCU polls the IMU over this 2-wire bus at ≥100Hz, reading raw 16-bit accelerometer/gyroscope register counts. No unit conversion happens on this board — the numbers are literal ADC counts.' },
  { from:'U1', to:'HOST', fromSide:'right', toSide:'top', category:'uart', net:'UART_TX', dir:1, yoff:125, xoff:-45,
    why:'One-way CSV telemetry out: timestamp + 6 raw IMU axes, sent at the same 100Hz the IMU is sampled. Nothing on this board reacts to this data — it is only ever reported outward.' },
  { from:'HOST', to:'U1', fromSide:'top', toSide:'right', category:'uart', net:'UART_RX', dir:1, yoff:145, xoff:15,
    why:'ASCII text commands typed by a bench operator, e.g. a motor speed setpoint. This is the only way the motor ever gets a target — there is no automatic or sensor-driven command path yet.' },
  { from:'U1', to:'U6', fromSide:'bottom', toSide:'top', category:'ctrl', net:'U6_EN (SHDN)', dir:1, xoff:-70, toXoff:0,
    why:'A single digital line that arms or disarms the entire motor rail. motor_init() leaves this LOW (disarmed) at boot — an operator must explicitly send a REARM command before U5 ever receives power.' },
  { from:'U1', to:'U5', fromSide:'bottom', toSide:'top', category:'ctrl', net:'SPEED_PWM / DIR', dir:1, xoff:-15, toXoff:-30,
    why:'The MCU\'s only two outputs to the motor driver: a PWM duty cycle (speed) and a direction bit. U5 handles all commutation internally — the MCU never needs to know rotor position.' },
  ...
  { from:'SW1', to:'U1', fromSide:'top', toSide:'left', category:'debug', net:'NRST', dir:0, yoff:80,
    why:'Pulls the reset line low while held — a hardware reset, equivalent to power-cycling the MCU.' },
  { from:'U1', to:'D1', fromSide:'left', toSide:'top', category:'debug', net:'LED_CTRL', dir:0, yoff:100,
    why:'Drives the heartbeat LED: 1Hz blink when the IMU is healthy, a faster blink if BMI270 init failed — lets a bench operator read board health at a glance, no terminal needed.' },
  { from:'U1', to:'J2', fromSide:'left', toSide:'top', category:'debug', net:'UART hdr', dir:0, yoff:120,
    why:'The physical 4-pin header itself, not a separate signal — electrically the same UART_TX/RX wires shown above, continuing out to whatever cable is plugged into J2.' },
  { from:'U1', to:'J3', fromSide:'left', toSide:'top', category:'debug', net:'SWD hdr', dir:0, yoff:140,
    why:'Programming/debug interface only, used to flash firmware. Electrically silent during normal operation — nothing here during the flows shown in Modes 1-2.' },

  // ---- future (mode 3 only) ----
  { from:'U2', to:'CTRL', fromSide:'bottom', toSide:'top', category:'future', net:'orientation error (future)', dir:1,
    why:'Hypothetical only — no code path today reads the IMU for this purpose. Right now the IMU\'s readings only ever travel out over UART_TX (Mode 2), never back toward the motor.' },
  { from:'CTRL', to:'U1', fromSide:'left', toSide:'right', category:'future', net:'corrected speed cmd (future)', dir:1, yoff:-60,
    why:'Hypothetical only — SPEED_PWM/DIR (Mode 2) are the ONLY real inputs to the motor driver today. A motor command computed from IMU error does not exist in firmware yet.' },
];
