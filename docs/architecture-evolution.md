...
## 44. Systems Engineer — Interface Control & Cross-Discipline Trade-off Judgment (Phase 7 of the multidisciplinary evolution)

Numbered 44 rather than 43 because §43 ("Diff-Aware CRITICAL/HIGH Exemption
for the Hardware Gate") was already claimed on `main` by PR #44
(`ktanino10-diff-aware-hardware-gate-exemption`, merged after this session's
own worktree branched off) -- confirmed by fetching `origin/main` and
resolving the resulting merge conflict in this same file directly, the same
class of cross-branch numbering collision `docs/workflow.md` §4.1 already
documents a resolution convention for (there, ECO/ISS/MISS/DS IDs; here, this
document's own section numbers), and the same convention PR #44's own §43
entry above just used for its own 42-vs-43 collision.
