...
and its original release manifest are unchanged. The current public base also
contains the separately published v3, six Blender movies and offline IMU tools;
their runtime payloads are preserved, not regenerated by this follow-up.

The original software and offline-tool manifests remain historical snapshots.
[software-continuation.json](software-continuation.json) binds each intentionally
updated support file to its old manifest hash and its current bytes. The shared
