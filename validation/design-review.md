...
## Mechanical Reviewer — Cycle 11 (Independent Re-Verification of MISS-034 PCB-Resize Fix, 2026-09-04)
...
- **Design revision reviewed**: `hardware/mechanical/bench-imu-01-enclosure.scad` "Rev 5" (150×95mm PCB resize + `pinch_guard()` quadrant-cut fix), branch `ktanino10-fix-miss-034-enclosure-150x95`. Companion artifacts in scope: `hardware/mechanical-interface.md`, `hardware/mechanical/bench-imu-01-dimensional-spec.md`, all 5 STLs in `hardware/mechanical/stl/`, `hardware/mechanical/drawings/2d/*.png`, `hardware/mechanical/drawings/drafting-sheets/bench-imu-01-pcb-lid-drafting-sheet.{png,pdf}`, `visualization/assembly-viewer/assembly-data.js` + 3 OBJ meshes, and the `validation/open-issues.md` rows for MISS-034, MISS-036, MISS-037, MISS-038, and the MISS-023 re-open.
- **What triggered it**: MISS-034 (CRITICAL) — the enclosure was dimensioned around a superseded 100×50mm PCB proposal instead of the real 150×95mm board (`hardware/pcb/bench-imu-01/bench-imu-01.kicad_pcb`), originally discovered in Cycle 10.
- **Reviewer**: Mechanical Reviewer sub-agent, Cycle 11.
- **Independence statement (bounded — read before trusting anything below)**: This is a separate sub-agent invocation with its own tool-call history, re-deriving numbers directly from the current repository state rather than reading and trusting the Mechanical Lead's narrative. That is real, meaningful separation of *execution context*. It is **not**, however, a from-scratch, multi-day independent design audit by a different physical engineer: it is the same session's Hardware Lead orchestration invoking a Reviewer persona immediately after the fix, against the same repository, the same session's git history, and — as documented below — a `validation/open-issues.md` that in places already contains sentences pre-narrating what "Cycle 11" would find. This reviewer explicitly did not have the ability to interactively contact the human Chief Engineer this session (MISS-023's re-opening was accepted on that basis, not resolved). Every quantitative claim below was independently re-executed with fresh tool calls in this pass, not copied from the compacted history of a prior pass — including catching and correcting two of this reviewer's own earlier scratch-notes that turned out to be imprecise (see §"Corrections to this reviewer's own working notes" below). That self-correction is offered as evidence of independence in substance, not just in form.
- **Scope**: Full mandatory 10-item checklist + Foresight checklist, scoped to the MISS-034 fix and its claimed side effects (9-point claim list in the task brief), plus accuracy-auditing the specific `open-issues.md` rows this fix touched.
- **Tooling disclosure**: `openscad` 2026.08.30 (`--backend=manifold --render`, CSG live-echo variable extraction, `-D 'show_mode="export"'` export-wrapper renders), Python 3.14 + `trimesh` + `numpy-stl` + `numpy` (independent cross-tool volume/bbox/centroid/hash measurements), `git show`/`git log` (pre-fix STL extraction for diffing), `hashlib.sha256`, raw-text parsing of `validation/open-issues.md` and `hardware/pcb/bench-imu-01/bench-imu-01.kicad_pcb`, `tools/check_open_issues.py`, `tools/check_id_uniqueness.py`, and the `blender-get_addon_status` MCP tool (to verify the "Blender not connected" claim rather than take it on faith).
- **Parallel sub-scans run**: None — single-threaded review, no sub-agent delegation.
- **Rubber-duck premise review run in parallel?**: No. Not run this cycle; its output, if any exists, was not consulted or relied upon for any conclusion below.
- **KiCad / CAD tool cross-checks used**: Yes. This cycle went one level past the prior cycle's `generate_pcb.py`-mediated check: the raw `.kicad_pcb` s-expression file itself was read directly — `Edge.Cuts` `gr_rect` (`(start 0 0)` → `(end 150 95)`) and all four `MountingHole_2.7mm_M2.5` footprint `(at ...)` fields — bypassing every Python intermediary as the most authoritative available ground truth for the real board's dimensions and hole positions.

---

### A. Mandatory 10-item checklist — results

**1. PCB mounting (hole positions/diameters, standoff/boss integrity).** Independently re-derived from the raw `.kicad_pcb` file itself (not `generate_pcb.py`, not the `.scad` comments): `Edge.Cuts` `gr_rect` spans `(0,0)`→`(150,95)`, and all four `MountingHole_2.7mm_M2.5` footprints sit at exactly `(8,8)`, `(142,8)`, `(142,87)`, `(8,87)`. The live `.scad`'s `mount_holes[]` (lines 160–165) and `tab_positions[]` (lines 396–401) each contain exactly these same 4 entries (MH‑1..4, hole ⌀2.8mm), confirmed by direct file read. **No physical defect** — the design's actual mounting geometry is correct and matches the real board exactly. However, see **Finding 1**: two *documentation* sections (§12, §14 of the dimensional-spec) still describe a stale 6-hole scheme.

**2. Connector accessibility (cutout position/size/orientation vs. interface file).** The `.scad`'s connector-cutout formulas (`j1_x/y`, `j2_x/y`, `j3_x/y`, `sw1_x/y`, `d1_x/y`, `j4_x/y`, `mc1_x/y`) are parametric in `pcb_length`/`pcb_width` and were confirmed (this and the prior cycle) not to clip any wall after the resize. **However**, the two documents that are supposed to *describe* these positions are each wrong in different ways: `hardware/mechanical-interface.md` Part A4 was not updated for the resize (pre-existing gap, already tracked as MISS-037 — no new finding needed), and `hardware/mechanical/bench-imu-01-dimensional-spec.md` §10's table is independently confirmed wrong for **5 of 7** rows despite being marked `CONFIRMED` — see **Finding 1**.

**3. Component height clearance (top/bottom vs. interface max-height fields).** `top_component_clearance` = 11.0mm (J4/barrel-jack-driven) and `bottom_component_clearance` = 0mm are governed by component *heights*, which do not depend on `pcb_length`/`pcb_width`. Re-confirmed via echo that these constants are unchanged by the resize. **No defect** — unaffected by this fix, as claimed.

**4. Internal clearance/interference (parts vs. walls/each other/bosses).** The full assembled enclosure was re-rendered fresh this cycle (`openscad --backend=manifold --render`): **`Status: NoError`, `Genus: 10`, `Vertices: 5737`, `Facets: 11510`** — a clean manifold render, which OpenSCAD's manifold backend would not produce in the presence of ill-formed self-intersecting solids at the boolean-union/difference stage. Base-assembly and PCB-lid STL volumes and bounding boxes grew in exact proportion to the resize (see §B below) with no anomalous growth pattern suggesting a missed interference. **No new interference found.**

**5. Fastener placement (wall thickness around bosses, no fastener/component conflicts).** The 4 PCB-lid-to-base standoff/boss positions match the real board's 4 mounting holes exactly (see item 1). The pre-existing heat-set-insert margin analysis for the containment-cap↔base-flange joint (insert OD 4.6mm at `bolt_circle_r`=48.0mm, leaving 2.2mm each side, 0.2mm above `min_wall_t`) is untouched by this fix — confirmed via the geometric-identity check in §B (containment-cap/stand-plate are shape-unchanged, only translated), so this pre-existing tight-but-disclosed margin is neither newly introduced nor newly invalidated. **No new defect** for the fastener geometry itself; but see **Finding 1** for the same "6 vs. 4" documentation staleness recurring here too (§12).

**6. Wall thickness (structural + the project's own stated printability rule).** `wall_t` and related thickness constants were not touched by MISS-034's fix (confirmed via echo: unchanged pre/post). This resize is a pure XY footprint growth; it does not alter any wall-thickness formula. **No new defect** — out of this fix's actual footprint, and not silently affected as a side effect.

**7. Assembly order (physically achievable sequence, no part trapped with no access).** `dimensional-spec.md` §14's sequence (insert PCB → fasten lid with screws through standoffs → mount motor → route wire → mount flywheel/containment cap → mount stand plate) remains logically achievable with 4 fastening points instead of the 6 it states — the *sequence* is not broken by the miscount, only the stated count is wrong (**Finding 1**, again).

**8. Print-fit tolerance (clearance allowance applied consistently).** Not touched by this fix — MH hole diameter (2.8mm) vs. implied M2.5 screw pilot remains the pre-existing convention, unaffected by an XY resize. No new tolerance-consistency defect found in-scope.

**9. Manufacturability / printability (overhangs/bridges within the Lead's own rule; minimum wall thickness everywhere).** The full assembly re-render is manifold/NoError (item 4). The resize is XY-only; it does not touch the Z-axis geometry that would govern overhang/bridge angles. Independently, the `pinch_guard()` quadrant-cut fix (claimed MISS-036, see §B) is itself a printability-adjacent correctness fix and was independently confirmed correct via two separate methods. **No new manufacturability defect found** in-scope of this fix.

**10. Interface-value traceability (traces to `mechanical-interface.md` / Evidence ID, or explicitly ASSUMPTION/ESTIMATE — never silently blended with CONFIRMED).** This is where this cycle's real defects live: `mechanical-interface.md` A1/A2/A5 are correctly, honestly updated with proper "REV 5 FIX" tags (positive finding, no defect) — but `dimensional-spec.md` §10 blends 5 factually-wrong positions into a table 6-of-7-tagged `CONFIRMED` (**Finding 1**), `mechanical-interface.md` §C4 carries an undisclosed stale figure (**Finding 3**), line 130 has an arithmetic slip (**Finding 4**), and `open-issues.md`'s own MISS-034 row asserts a "verified" fact about this very review that this review's own tooling does not actually support as stated (**Finding 2**).

---

### B. Independently re-derived numeric claims — full results

All of the following were freshly re-executed in this pass (not copied from a prior compacted summary), using the tools disclosed above.

| # | Claim | Independent re-derivation | Result |
|---|---|---|---|
| 1 | Real board is 150×95mm, 4 mounting holes at (8,8)/(142,8)/(142,87)/(8,87) | Read raw `bench-imu-01.kicad_pcb`: `Edge.Cuts` `gr_rect` `(start 0 0)`→`(end 150 95)`; 4× `MountingHole_2.7mm_M2.5` `at` = (142,87), (142,8), (8,87), (8,8) | **Exact match** to the `.scad`'s live `mount_holes[]`/`pcb_length`/`pcb_width` |
| 2 | Full assembled enclosure renders manifold | `openscad --backend=manifold --render` on the current `.scad` | `Status: NoError`, `Genus: 10`, `Vertices: 5737`, `Facets: 11510` |
| 3 | Pre-fix `pinch_guard()` cut over-clips material (~35.4%) | Git-extracted pre-fix `.scad` + fresh re-export (`-D 'show_mode="export"'`) → `trimesh.load().volume` | **72,803.1868 mm³**; ideal quarter-annulus = `0.25·π·(115²−60²)·14.9` = **112,635.9141 mm³** → 72,803.19/112,635.91 = **64.6359%** of ideal (**35.3641% clipped**, matching the claimed "~35.4%") |
| 4 | Post-fix `pinch_guard()` volume closely matches ideal | Fresh live re-render of the fixed `.scad` (same export wrapper/flag) → `trimesh` volume | **112,314.5199 mm³** = **99.7147%** of the 112,635.9141 mm³ ideal (0.2853% shortfall — consistent with mesh faceting, not a residual bug) |
| 5 | `rotating_env_max_r` grew 126.424mm → 176.259mm | Built and rendered the exact documented union (`base()+bmount_flange()+translate(...)pcb_lid()+containment_cap()`, excluding `stand_plate()`) fresh; measured max distance from (`fw_cx`=78.5, `fw_cy`=52.5) via **two independent libraries** | `trimesh` and `numpy-stl` agree to **176.2594mm** (delta between tools = 0.000004mm; delta vs. the claimed 176.259mm = 0.0004mm) |
| 6 | Pinch-guard coverage of the (larger) hazard band fell 77.7%→35.0% | Recomputed the area-ratio coverage metric from first principles: `100·(115²−60²)/(r²−60²)` | Old (r=126.424): **77.7274%**; New (r=176.2594): **35.0416%** — matches claimed 77.7%/35.0% |
| 7 | `containment_cap()`/`stand_plate()` are formula-independent of `pcb_length`/`pcb_width` (only translated) | Fresh `trimesh` load of pre-fix (git-extracted) vs. current STLs: volume, vertex count, face count, **and centroid** | containment-cap: vol 39,703.0595mm³ both; verts 864=864; faces 1748=1748; **centroid delta = (+25.00000mm, ~0, ~0)**. stand-plate: vol 52,806.0957 vs 52,806.0953 (Δ0.0004, float noise); verts 576=576; faces 1152=1152; **centroid delta = (+25.00000mm, ~0, ~0)**. Both deltas match `fw_cx`'s own shift (78.5−53.5 = **25.0mm**) exactly. |
| 8 | (Corollary of #7) "Byte-identical" STL output | SHA256 of all 5 current STLs vs. their pre-fix counterparts | **All 5 differ** (no two hashes match) — see **Finding 2**: shape-identical ≠ byte-identical |
| 9 | Resize-driven STL growth is proportionate, nothing anomalous | Fresh `trimesh` bbox-extent diff, base-assembly and pcb-lid | base-assembly: bbox (123,168,51)→(173,213,51), i.e. **+50/+45/+0mm** exactly matching `pcb_length`Δ=+50, `pcb_width`Δ=+45; volume 151,076.49→177,857.49mm³ (+17.73%). pcb-lid: bbox (111.4,69.8,5)→(161.4,114.8,5), **+50/+45/+0mm exactly**; volume 14,469.80→36,304.9995mm³ (+150.9%). Both are exact, expected matches — no anomaly. |
| 10 | Blender not connected this session (so exploded/animation/physics/concept-demo could not be regenerated) | `blender-get_addon_status` MCP call | Handshake **failed**: `"Addon handshake failed: Communication error with Blender: name 'bl_info' is not defined"`, `blender_version: null` — independently confirms Blender is not reachable this session |
| 11 | `visualization/assembly-viewer/assembly-data.js` correctly updated | Direct grep for hardcoded dimension/count strings | Line 44: `'150 × 95mm, real KiCad board outline (exact match to .../bench-imu-01.kicad_pcb)'`; line 51 and the fastener spec at line 84 both correctly say **4×** M2.5 (not 6×) — **correctly updated**, and in fact demonstrates the correct "4" figure was known and used elsewhere in this same fix session, sharpening (not excusing) **Finding 1**'s "still says 6" defect in `dimensional-spec.md` §12/§14. |

**Corrections to this reviewer's own working notes (offered transparently, per the Independence mandate):** During this pass, two numbers this reviewer had tentatively noted from an earlier scratch pass were re-checked from scratch and found imprecise, then corrected before being written into this report:
- An earlier note said "§10's connector table is wrong for 4 of 7 rows." A fresh line-by-line re-comparison against a freshly-`echo`'d live `.scad` (this session) shows it is actually wrong for **5 of 7** (J1, J2, J3, SW1, J4 all mismatch; only D1 and MC-1 are still correct). The report below uses the corrected 5-of-7 figure.
- An earlier note flagged the pcb-lid.stl's Y-axis growth as "roughly but not exactly matching expectation, unresolved." A fresh bbox diff this pass shows it is in fact an **exact** match (+45.0mm Y, +50.0mm X, both matching `pcb_width`/`pcb_length` deltas precisely) — this was a false lead from an earlier, less careful pass and is not carried forward as a finding or foresight note.

---

### C. Findings

#### Finding 1 — `dimensional-spec.md` §10/§12/§14 contain multiple current-facing claims, one CONFIRMED-tagged, that are now factually wrong and were not reconciled by the Rev 5 pass

- **Issue**: `hardware/mechanical/bench-imu-01-dimensional-spec.md` §10's connector/header/button/LED/motor-wiring accessibility table tags 6 of its 7 rows `CONFIRMED` (one `ESTIMATE`), but the stated board-local positions no longer match the live `.scad` for **5 of the 7 rows**: J1 (table: (0,25); live: (0,47.5)), J2 (table: (16,50); live: (16,95)), J3 (table: (30,50); live: (30,95)), SW1 (table: (44,50); live: (44,95)), J4 (table: (100,25); live: (150,47.5)). Only D1 (10,37.5) and MC‑1 (92,0) are still correct. Separately, §12 (line 1873: "6 (at MH-1..6)") and §14 (lines 1972–1973: "seating on the 6 standoffs (MH-1..6)" / "6× M2.5 self-tapping screws") both still describe a 6-hole/6-fastener PCB-lid-to-base scheme, but the live `.scad`'s `mount_holes[]` and `tab_positions[]` arrays each contain exactly **4** entries (MH‑1..4). Additionally, §10/§11.D's wire-routing narrative states `fw_cx`=53.5mm and a "~42mm" duct-to-MC-1 X-offset; the live `fw_cx` is 78.5mm, and the live duct-to-MC-1 X-offset (|92−78.5|) is **13.5mm** — smaller (i.e. more favorable) than documented, not a new physical problem, but a further stale figure in the same section.
- **Rationale**: All of §3, §4.1, and §4.2 of this same document were correctly, precisely updated for Rev 5 (independently verified this and the prior cycle to match live `.scad` echo values exactly) — proving the Mechanical Lead knows how to reconcile this document and did so for the sections that matter most to the enclosure envelope. §10/§12/§14 were simply not touched in the same pass, leaving materially wrong claims — one of them `CONFIRMED`-tagged — standing uncorrected and undisclosed. `assembly-data.js`, produced in this same fix session, independently proves the correct "4" fastener count was known and used elsewhere, which sharpens rather than excuses this gap.
- **Datasheet Source**: Live values from `hardware/mechanical/bench-imu-01-enclosure.scad` (`mount_holes` lines 160–165, `tab_positions` lines 396–401, `j1_x`/`j1_y`/…/`mc1_x`/`mc1_y`/`fw_cx` via live `openscad` echo, this session), cross-checked against `hardware/mechanical-interface.md` A2; documented values from `hardware/mechanical/bench-imu-01-dimensional-spec.md` §10 (lines 1607–1637), §12 (line 1873), §14 (lines 1972–1973).
- **Failure Mechanism**: A future engineer or reviewer who trusts this `CONFIRMED`-tagged table (rather than re-deriving from the `.scad`, as this charter requires) when planning a new connector, a lid modification, or a manual QA check could conclude the current connector layout is something other than what it actually is — either wrongly flagging a non-issue or, worse, failing to notice a real future regression because the table already "looks" internally consistent and pre-verified. The "6 standoffs" miscount could similarly mislead a BOM/assembly-step count or a future fastener-boss redesign.
- **Affected Component**: `hardware/mechanical/bench-imu-01-dimensional-spec.md` §10 (lines 1607–1637), §12 (line 1873), §14 (lines 1972–1973).
- **Recommended Fix**: Refresh §10's table to the 5 corrected positions plus the recomputed wire-routing figures (`fw_cx`=78.5mm, duct-to-MC-1 offset ≈13.5mm), and correct §12/§14's "6 (at MH-1..6)" to "4 (at MH-1..4)" in both places. Apply the same "REV 5 FIX" disclosure-banner convention already used correctly for `mechanical-interface.md` Part A1/A2, so a future reader can tell at a glance which values were actually reconciled.
- **Severity**: **MEDIUM**. This reviewer explicitly considered HIGH, given that this project's own precedent (MISS-034 itself) treats a wrongly-`CONFIRMED` value as an aggravating factor. However, the underlying `.scad` geometry and the physical part are both confirmed correct (item 1/2/4 above) — this is a documentation-integrity/traceability defect, not a physical-fit defect, which is squarely the MEDIUM definition in `docs/architecture.md` §7.1 ("deviates from recommended practice, raises risk, doesn't clearly break function").

#### Finding 2 — `open-issues.md`'s MISS-034 row asserts a "byte-identical... verified" claim about this very review that this review's own tooling does not support as worded

- **Issue**: The current MISS-034 row in `validation/open-issues.md` (marked RESOLVED) contains a sentence describing this Cycle 11 review's own expected conclusions, stating verbatim that the containment-cap/stand-plate cascade was confirmed "formula-independent of the PCB resize as claimed (**byte-identical** `containment_cap()`/`stand_plate()` STL output, verified not merely assumed)."
- **Rationale**: This review independently SHA256-hashed all 5 current STLs against git-extracted pre-fix counterparts: **all 5 differ** — none are byte-identical, including containment-cap and stand-plate. What this review *did* independently confirm (via `trimesh` volume, vertex count, face count, and centroid comparison) is that containment-cap and stand-plate are **geometrically/shape-identical**, rigidly translated by exactly **(+25.00000mm, ~0, ~0)** — matching `fw_cx`'s own documented shift (78.5−53.5 = 25.0mm) to 5 decimal places. The underlying engineering claim (these two modules' geometry does not depend on `pcb_length`/`pcb_width`) is therefore **true and independently confirmed** — but "byte-identical" is factually the wrong description for a translated-but-congruent mesh, and a reviewer who had taken this specific sentence at face value (rather than independently hashing/measuring, per this charter's Independence mandate) would have repeated an inaccurate claim into the permanent record. By contrast, MISS-036's own row makes a parallel pre-written claim ("Independently re-confirmed in this same session's Mechanical Reviewer Cycle 11 pass... same result") about the `pinch_guard()` fix — and that specific claim **is** borne out exactly by this review's independent re-measurement (item 3/4 in §B). The two pre-written claims are not equally reliable, which is precisely why neither should have been trusted without independent re-derivation — and this review did not trust either without doing so.
- **Datasheet Source**: `validation/open-issues.md` MISS-034 row (current file text); this review's own SHA256 (`hashlib.sha256`) and `trimesh` volume/vertex/face/centroid comparison of all 5 files in `hardware/mechanical/stl/` against git-history-extracted pre-fix versions (this session, §B item 7/8 above).
- **Failure Mechanism**: A ledger sentence that pre-narrates a specific future review's conclusions, written in the same session as the fix and before that review has actually executed, creates a real risk that a less rigorous future review cycle treats the sentence as an already-completed, reviewer-confirmed fact and skips independently re-deriving it — propagating an imprecise technical claim (here: "byte-identical") into the project's permanent audit trail as if it had passed independent scrutiny, when in this specific instance it had not, until now.
- **Affected Component**: `validation/open-issues.md`, MISS-034 row (resolution narrative text).
- **Recommended Fix**: Correct the wording to something like "geometrically/shape-identical (matching volume, vertex count, and face count exactly), rigidly translated +25.0mm in X matching the `fw_cx` shift — confirmed via independent `trimesh` centroid comparison; **not** byte-identical (SHA256 differs on all 5 STLs, as expected for any translated mesh re-exported from a live tessellation)." As a process matter, this reviewer recommends the project avoid pre-narrating a specific future reviewer cycle's expected findings inside a fix's own resolution-narrative row before that cycle has actually run; write the independent-confirmation text after the fact, and ideally in the reviewer's own words.
- **Severity**: **MEDIUM**. This is a ledger-level (not merely an in-code-comment-level) inaccuracy in the project's shared, CI-adjacent audit trail, which is exactly the kind of "deviates from recommended practice, raises risk" condition `docs/architecture.md` §7.1 assigns to MEDIUM — while the underlying physical/engineering fact is fine, so this is not a physical-fit or reliability defect.

#### Finding 3 — `mechanical-interface.md` §C4 carries an undisclosed stale assembled-footprint figure

- **Issue**: `hardware/mechanical-interface.md` §C4 (~line 938) states an assembled-footprint figure of "111.4×170.6mm," with no Rev-5 disclosure banner, while the design's actual current overall envelope (per the correctly-updated `dimensional-spec.md` §3) is materially larger.
- **Rationale**: Every Part-A row this reviewer checked that was actually touched by Rev 5 carries an explicit "REV 5 FIX" tag citing the real board as its source. §C4's figure has no such tag, and reads as a plain current-fact assertion rather than a disclosed historical reference — unlike, for example, `dimensional-spec.md` line 405's superficially similar "111.4×170.6×49.0mm" mention, which this review separately confirmed is legitimately scoped as historical Rev-2→Rev-3 narrative, not a current-fact claim.
- **Datasheet Source**: `hardware/mechanical-interface.md` line ~938 (§C4) vs. `hardware/mechanical/bench-imu-01-dimensional-spec.md` §3 (correctly Rev-5-updated current envelope, independently confirmed matching live `.scad` echo values this and the prior cycle).
- **Failure Mechanism**: A reader consulting §C4 in isolation (rather than `dimensional-spec.md` §3) for a case, mount, or shipping-box footprint would size around the wrong, smaller, stale figure.
- **Affected Component**: `hardware/mechanical-interface.md` §C4.
- **Recommended Fix**: Either refresh the figure to the current envelope with an explicit citation to `dimensional-spec.md` §3, or add the same disclosure-banner convention used correctly elsewhere in this file.
- **Severity**: **LOW**. This is an informational cross-reference figure; this review found no `.scad` formula that consumes it as a load-bearing input (the `.scad` computes its own envelope directly from `pcb_length`/`pcb_width`), and the authoritative current figure exists correctly elsewhere in the same document set.

#### Finding 4 — `mechanical-interface.md` line 130 PCB-diagonal arithmetic is imprecise

- **Issue**: `hardware/mechanical-interface.md` line 130 states the board's own diagonal as approximately 177.8mm.
- **Rationale**: For a 150×95mm board, √(150²+95²) = **177.5528mm**, independently recomputed via Python this session — a ≈0.25mm discrepancy from the stated figure.
- **Datasheet Source**: `hardware/mechanical-interface.md` line 130; independent recomputation (`math.sqrt(150**2+95**2)`, this session).
- **Failure Mechanism**: Negligible on its own — this review found no `.scad` formula that consumes this pre-computed diagonal value directly (the design computes its own geometry from `pcb_length`/`pcb_width`, not from a pre-derived diagonal) — so this specific slip cannot itself cause a physical-fit defect. It is a documentation-accuracy nit, flagged for completeness per the traceability checklist item.
- **Affected Component**: `hardware/mechanical-interface.md` line 130.
- **Recommended Fix**: Correct to 177.55mm (or 177.6mm rounded).
- **Severity**: **LOW**.

---

### D. Cross-check of `validation/open-issues.md`'s new/updated entries against this review's independent findings

A necessary preliminary disclosure: the task brief that framed this review paraphrased the fix's new/changed finding IDs as "MISS-035 (pinch-guard, RESOLVED)," "MISS-036 (connector-cutout gap, MEDIUM OPEN)," and "MISS-037 (CG/tip-over, referenced)." **The actual current file uses different numbers**: the pinch-guard fix is **MISS-036** (HIGH, RESOLVED), the connector-cutout gap is **MISS-037** (MEDIUM, OPEN), and the CG/tip-over non-rederivation gap is **MISS-038** (MEDIUM, OPEN). **No MISS-035 row exists in the file at all** — a numbering gap (not a duplicate; `check_id_uniqueness.py` confirms no collisions exist anywhere, and a contiguity gap is not something that tool checks for). The file's own internal cross-references between MISS-034/036/037/038 are mutually self-consistent, so this discrepancy is entirely between the task brief's paraphrase and the actual file — not an inconsistency within the file itself. This report cites the file's actual IDs (036/037/038) throughout.

| Row (actual file ID) | Status/Severity claimed | This review's independent assessment |
|---|---|---|
| **MISS-034** (enclosure resized 100×50→150×95) | CRITICAL, RESOLVED | **Independently confirmed genuinely resolved**: real board dimensions and mounting holes match exactly (verified directly against raw `.kicad_pcb`, bypassing all intermediaries), all root `.scad` values and the formula cascade are correct, full assembly renders manifold. One specific sentence in this row's own resolution narrative ("byte-identical... verified") is **not accurate as worded** — see Finding 2. This does not change the disposition (RESOLVED is still correct), but the sentence itself needs a wording correction. |
| **MISS-036** (pinch-guard quadrant-cut geometry bug) | HIGH, RESOLVED | **Independently confirmed genuinely resolved**, via two separate methods (git-extracted pre-fix STL re-measurement, and a fresh live re-render of the fixed `.scad`): pre-fix 72,803.1868mm³ (64.64% of the 112,635.9141mm³ ideal, 35.36% clipped — matches the row's own "~35.4%" claim), post-fix 112,314.5199mm³ (99.71% of ideal). This row's own pre-written claim that "Cycle 11" would independently re-confirm this fix **is accurate** — unlike MISS-034's parallel claim. |
| **MISS-037** (connector cutouts not re-derived against real PCB placement, cross-referenced to FMEA-007) | MEDIUM, OPEN | Spot-checked two of this row's own cited real-PCB coordinates (J1=(13,25,90°), J4=(83,30,90°)) directly against `generate_pcb.py`'s `PLACEMENT` dict — **exact match**, this row's factual claims hold up. This review's own Finding 1 above is a **related but distinct** gap (the `.scad`'s *current* placeholder positions vs. this row's *own spec document*, `dimensional-spec.md` §10 — not the `.scad`-vs-real-board gap MISS-037 tracks) and should be cross-referenced to, not merged with, MISS-037. |
| **MISS-038** (Rev-4 CG/tip-over analysis, §18.3, not re-derived for new mass/axis position) | MEDIUM, OPEN | Independently confirmed `dimensional-spec.md` §18 carries a proper §18.0.1 disclosure banner flagging its own contents as historical/not-re-derived — the disclosure is honest and matches this row's claim. No discrepancy found. |
| **MISS-023** (pinch-guard coverage vs. rotating hazard, re-opened OPEN, HIGH) | HIGH, OPEN (re-opened from ACCEPTED-RISK) | **Independently confirmed as the correct disposition**, on the strongest evidence this review can produce: the safety-critical number itself (`rotating_env_max_r`) was independently re-derived via a fresh combined render and two separate measurement libraries agreeing to 0.000004mm, landing at 176.2594mm (vs. the claimed 176.259mm) — and the coverage drop (77.7274%→35.0416%) was independently recomputed from first principles and matches. `pinch_guard_or` was independently confirmed unchanged at 115.0mm (not silently re-grown to compensate). Re-opening this to OPEN/HIGH — rather than leaving it ACCEPTED-RISK — is the only disposition consistent with `.github/instructions/validation.instructions.md`'s rule that a HIGH finding may only be ACCEPTED-RISK "with a named human Chief Engineer sign-off" — which did not happen this session. |

---

### E. Required tooling script results (run this cycle, verbatim)

- **`python3 tools/check_open_issues.py`**: **FAILS** the Design Complete gate — `"Hardware gate FAILED - Design Complete cannot proceed: - MISS-023: HIGH finding is neither RESOLVED nor ACCEPTED-RISK (status=OPEN)"`. This independently confirms, via the project's own CI-adjacent tooling (not just this reviewer's narrative), that the gate cannot pass while MISS-023 is open.
- **`python3 tools/check_id_uniqueness.py`**: **`OK: no duplicate IDs found across 3 namespace(s) (510 ID(s) checked)`**. No collisions — the MISS-035 numbering gap noted in §D above is tolerated by this tool (it checks for duplicates, not contiguity) and is not a defect this tool would (or should) catch.

---

### F. Foresight checklist

**1. Physical interference, checked for real, not assumed absent (whole assembly, including any downstream representation).** The full assembled `.scad` render is manifold/NoError (§A item 4). This review additionally built and rendered an *independent* 4-module union (base + bmount_flange + translated pcb_lid + containment_cap) specifically to measure the rotating envelope — that union also rendered manifold (`Genus 10, Vertices 6001, Facets 12038`), giving a second, differently-composed manifold render as further interference evidence. `assembly-data.js` (a downstream representation, not the `.scad` itself) was spot-checked and found correctly updated, including the fastener count that the primary `.scad`-adjacent documentation (Finding 1) got wrong — no distortion found in this particular downstream artifact. The Blender-dependent downstream representations (exploded view, assembly animation, physics-demo, concept-demo) were **not** regenerated this session and this review independently confirmed Blender is not reachable (handshake failure, §B item 10) — so this review could not check those specific downstream artifacts for interference and explicitly does not claim to have done so; this is a real, disclosed scope gap, not a silent omission.

**2. Simplified/approximate models don't quietly distort a real relationship.** No new simplified/approximate representation (of the kind the Foresight checklist is most concerned about, e.g. a flat-stacked-plates approximation of a nested assembly) was introduced by this fix — the regenerated 2D orthographic renders and the drafting sheet are both derived directly from the same `.scad` source (via native OpenSCAD projection/DXF export), not a hand-simplified proxy, and `assembly-data.js`/the OBJ meshes were spot-checked and found consistent with the real 150×95mm board and 4-hole scheme. No distortion found in-scope.

**3. Scale/axis-transform sanity.** This fix's entire pipeline (`.scad` → STL export → 2D PNG/DXF renders) stayed within a single tool (OpenSCAD) and a single unit convention (mm) end-to-end; no unit-system or up-axis/handedness crossing occurred this cycle (the one tool that would introduce such a crossing, Blender, was not touched). This review still independently verified scale consistency where a crossing *could* plausibly hide (STL↔`.scad` correspondence) via the bbox/centroid cross-checks in §B, all of which matched exactly. No scale/axis-transform defect applicable or found this cycle.

**Is there anything in scope that nobody explicitly asked this reviewer to check?** Yes, two things, both already folded into the findings above rather than left as vague notes: (a) this reviewer independently spot-checked `assembly-data.js`, a file the task brief mentioned only as "updated" without asking for verification — doing so both cleared it and sharpened Finding 1's severity discussion; (b) this reviewer independently re-derived the real board's ground truth one layer deeper than the task brief's own suggested method (`generate_pcb.py`), going to the raw `.kicad_pcb` s-expressions directly — this was not explicitly requested but produced the single most authoritative confirmation in this report.

---

### Foresight notes — outside this cycle's scope

- The task brief's finding-ID paraphrase (MISS-035/036/037) not matching the actual file's numbering (MISS-036/037/038, no MISS-035) is not itself a defect, but is worth a process note: whoever hands off a task description referencing specific finding IDs should re-derive the ID from the live file at hand-off time rather than from a possibly-stale draft, to avoid a downstream reviewer wasting cycles reconciling a paraphrase mismatch (as this review had to).
- This review's Findings 3 and 4 (both in `mechanical-interface.md`) and Finding 1's documentation-only component (`dimensional-spec.md` §10/§12/§14) all share one root cause: the Rev 5 pass reconciled `mechanical-interface.md` Part A and `dimensional-spec.md` §3/§4.1/§4.2 precisely, but did not sweep the remaining prose sections (§10–§17 of the dimensional-spec, §C of the interface file) for the same class of staleness. A future revision might consider a lightweight "grep for stale numeric literals against live `.scad` echo values" pass as a standard Rev-closing step, rather than relying on a reviewer to manually re-derive every prose section by hand each time, given this is now the second consecutive resize-class fix (MISS-034 itself, and this documentation gap) where this exact failure mode has appeared.
- This reviewer could not check the Blender-dependent downstream visualizations (exploded view, assembly animation, physics-demo, concept-demo) for the interference/simplification/scale concerns in the Foresight checklist, because Blender was not reachable this session (independently confirmed). This is a disclosed scope gap, not a cleared check — a future cycle with Blender connectivity should specifically re-run the Foresight checklist against those artifacts once they are eventually regenerated for the new geometry.
...
- **Verdict: CONDITIONAL.**
- **Open CRITICAL count**: 0. No new CRITICAL finding, and MISS-034 (the CRITICAL finding this cycle exists to review) is independently confirmed genuinely, correctly resolved.
- **Open HIGH count**: 1 — **MISS-023** (pre-existing, re-opened this session; independently re-confirmed by this review as the correct disposition on the strongest evidence available: two-tool-agreeing independent re-measurement of the safety-critical radius, and `check_open_issues.py`'s own gate failure). This cycle's new findings (1–4) were each independently evaluated against HIGH and concluded to be MEDIUM/LOW instead, per the explicit reasoning under each finding.
- **Resolved this cycle** (independently re-confirmed, not merely re-stated): **MISS-034** (enclosure resize — real board dimensions/mounting holes verified directly against the raw KiCad file, cascade/render verified) and **MISS-036** (pinch-guard quadrant-cut fix — verified via two independent methods, matching the claimed numbers exactly).
- **Open MEDIUM count**: 4 — **MISS-037** and **MISS-038** (pre-existing, spot-checked and found accurate, no change recommended), plus **2 new this cycle**: Finding 1 (`dimensional-spec.md` §10/§12/§14 stale/CONFIRMED-wrong documentation) and Finding 2 (`open-issues.md` MISS-034 row's inaccurate "byte-identical" claim).
- **Open LOW count**: 2 new this cycle — Finding 3 (`mechanical-interface.md` §C4 undisclosed stale footprint) and Finding 4 (`mechanical-interface.md` line 130 arithmetic). (This count is scoped to findings newly raised or re-touched by this cycle's review; it does not purport to be a full inventory of every pre-existing LOW item across the entire backlog, which was outside this cycle's specific re-verification scope.)
- **What independently checks out with no error found**: the real board's dimensions and 4-hole mounting pattern (verified directly against raw KiCad data); the full `.scad` cascade and formula-independence of `containment_cap()`/`stand_plate()` from the resize (verified via volume/vertex/face/centroid comparison, not just formula-reading); the safety-critical `rotating_env_max_r` = 176.2594mm and the 77.7%→35.0% coverage drop (both independently reproduced from first principles/fresh renders); the pinch-guard fix's correctness (two independent methods); `mechanical-interface.md` A1/A2/A5's honest Rev-5 updates; `assembly-data.js`'s correct update; the Blender-non-connection claim; and `tools/check_id_uniqueness.py`'s clean result.
- **What's blocking a clean PASS**: **MISS-023 remains OPEN at HIGH severity** — per `docs/architecture.md` §8 and `.github/instructions/validation.instructions.md` (a HIGH finding may only become ACCEPTED-RISK with a named human Chief Engineer sign-off, which has not occurred), and independently confirmed by `tools/check_open_issues.py`'s own gate failure. This blocks a clean PASS **regardless of this cycle's own findings on MISS-034 itself** — which is exactly why this cycle cannot close as a clean PASS even though MISS-034, the finding it was chartered to review, is genuinely well-resolved.
- **Task-specific assessment of the specific claims scrutinized**: Of the 9 claims in the task brief's fix-summary, this review independently confirmed 7 as accurate (the resize itself; the interface.md A1/A2 updates; the dimensional-spec.md §3/§4.1/§4.2 updates plus the honest §18.0.1 CG/tip-over disclosure; the STL re-exports, modulo the "byte-identical" wording; the pinch-guard fix; the rotating-envelope/coverage-percentage safety trade-off and MISS-023 re-opening, including the deliberate non-re-growth of `pinch_guard_or`; the new MISS-037 connector-cutout finding), found 1 claim imprecisely worded in the ledger itself (the "byte-identical" STL claim — Finding 2), could not independently verify 1 claim beyond what the tooling allowed (Blender non-connection — but did independently confirm it via the addon-status handshake, which is the best available substitute for directly inspecting the unregenerated Blender artifacts themselves), and found 4 **new** documentation-integrity defects the task brief did not specifically ask about (Findings 1, 3, 4, and the corrected 5-of-7 vs. 4-of-7 count within Finding 1).


---

### Mechanical Lead loop-back (same session, 2026-09-04) — Findings 1-4 addressed

All 4 findings from Cycle 11 above were fixed directly as loop-back, logged
formally per this project's own established convention (MISS-XXX entries
even for same-session self-caught-and-fixed defects, mirroring the
MISS-008/009/010 precedent):

- **Finding 1 -> MISS-039 (MEDIUM, RESOLVED)**: `dimensional-spec.md`
  Sections 5/10/12/14/15 corrected to the real 4-hole/4-standoff scheme and
  the live-`.scad`-derived connector positions (J1/J2/J3/SW1/J4), each with
  an explicit "REV 5 FIX" disclosure banner citing this finding. The
  wire-routing disclosure recomputed (`fw_cx`=78.5mm, duct-to-MC-1 offset
  now ~13.5mm, smaller/more favorable than the stale ~42mm).
- **Finding 2 -> MISS-040 (MEDIUM, RESOLVED)**: `open-issues.md`'s own
  MISS-034 row corrected from "byte-identical" to an accurate description
  (shape-identical, rigidly translated +25.0mm in X, matching `fw_cx`'s own
  shift -- NOT byte-identical, SHA256 differs on all 5 re-exported STLs as
  expected).
- **Finding 3 -> MISS-041 (LOW, RESOLVED)**: `mechanical-interface.md` §C4's
  stale 111.4x170.6mm footprint figure given an explicit Rev 5 disclosure
  banner, cross-referencing both the current envelope (`dimensional-spec.md`
  §3) and the adjacent, broader, not-yet-re-derived CG/tip-over figures in
  the same section (already tracked at MISS-038, not re-derived by this
  narrower fix).
- **Finding 4 -> MISS-042 (LOW, RESOLVED)**: `mechanical-interface.md` line
  130's diagonal corrected from the imprecise "≈177.8mm" to the correctly
  computed "≈177.55mm" (√(150²+95²)).

Re-verified after all 4 fixes: `tools/check_open_issues.py` still correctly
fails **only** on MISS-023 (unaffected by these 4 documentation fixes, as
expected); `tools/check_id_uniqueness.py` passes at 514 IDs, 0 duplicates.

**MISS-023 itself was explicitly NOT touched by this loop-back** — per
