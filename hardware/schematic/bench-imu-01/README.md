...
| J1 | USB-C receptacle | `Connector:USB_C_Receptacle_USB2.0_16P` | `Connector_USB:USB_C_Receptacle_GCT_USB4105-xx-A_16P_TopMnt_Horizontal` | CONFIRMED footprint + MPN (**updated — ISS-042/ECO-043**); symbol remains a generic-family choice (disclosed). J1's MPN is no longer an open item: the BOM-sourcing pass resolved it to the real, orderable **GCT USB4105-GF-A** (DS-CONN-007), and that evidence rests on *this exact footprint's* own embedded `tags` metadata listing that part number — so symbol-side footprint and BOM MPN now agree by construction. (Superseded history, kept for traceability: this row previously read "the design doc never formally selects J1's MPN (§3.1)", true only until DS-CONN-007 closed `bench-imu-01-design.md` §14 item 8. The schematic symbol's own `Value` field lagged that resolution and still read a **non-existent** `USB4105-GH-A` until ISS-042 corrected it — see `validation/open-issues.md`.) The *symbol* is still deliberately the generic `Connector:USB_C_Receptacle_USB2.0_16P`, not a GCT-specific symbol: it was chosen to match the doc's own "D+/D- present on the connector but left unpopulated" language (§3.2/§12) — a 16-pin USB2.0-only symbol has real D+/D-/SBU pins to leave NC, unlike the design doc's illustrative-only GCT USB4125 reference (a real 6-pin *power-only* part with no D+/D- pins at all). That generic-symbol choice is unaffected by the MPN resolution and needs no change: pin count/identity match USB4105-GF-A's 16-contact USB2.0 pinout |
