...
| J1 | USB-C receptacle | `Connector:USB_C_Receptacle_USB2.0_16P` | `Connector_USB:USB_C_Receptacle_GCT_USB4105-xx-A_16P_TopMnt_Horizontal` | CONFIRMED footprint + MPN (**updated — ISS-042/ISS-044, ECO-043**); symbol remains a generic-family choice (disclosed). J1's MPN is no longer an open item: the BOM-sourcing pass resolved it to the real, orderable **GCT USB4105-GF-A** (DS-CONN-007), closing `bench-imu-01-design.md` §14 item 8. **Land-pattern evidence is GCT's own manufacturer drawing** (https://gct.co/files/drawings/usb4105.pdf), against which this footprint was verified pad-for-pad in Hardware Reviewer Cycle 11: 22 pads = 16 SMD contacts + 4 through-hole shell stakes + 2 NPTH alignment pegs, pad order an exact match to the drawing's mating view, with the blank-suffix 0.95mm shell stake correct for this board's `(thickness 1.6)`. The KiCad footprint's own embedded `tags` metadata also names this MPN, but that is **corroboration only** — a CAD library's tag string is a community-maintained secondary source and is circular as primary evidence (the footprint cannot justify the part number that justifies the footprint), see ISS-044. (Superseded history, kept for traceability: this row previously read "the design doc never formally selects J1's MPN (§3.1)", true only until DS-CONN-007; and the schematic symbol's own `Value` field lagged that resolution and still read a **non-existent** `USB4105-GH-A` until ISS-042 corrected it.) The *symbol* is still deliberately the generic `Connector:USB_C_Receptacle_USB2.0_16P`, not a GCT-specific symbol: it was chosen to match the doc's own "D+/D- present on the connector but left unpopulated" language (§3.2/§12) — a 16-pin USB2.0-only symbol has real D+/D-/SBU pins to leave NC, unlike the design doc's illustrative-only GCT USB4125 reference (a real 6-pin *power-only* part with no D+/D- pins at all). That generic-symbol choice is unaffected by the MPN resolution and needs no change: pin count and pin identity match USB4105-GF-A's 16-contact USB2.0 pinout, independently confirmed in Cycle 11 |
