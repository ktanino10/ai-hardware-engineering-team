...
| 5 | `bench-imu-01-pinch-guard-quadrant.stl` | `pinch_guard(0)` | **4** (all four quadrants are geometrically identical — verified by direct inspection of the module body, a perfectly rotationally-symmetric ring cut at four evenly-spaced 90° angles; this is not 1-of-4-different-shapes) | Flat | Rev 4.1 — the MISS-023 pinch-point-hazard mitigation; **GROWN to full closure in Rev 5** (this same session). `pinch_guard_or` grew 115.0mm → 176.3mm — a **human Chief Engineer direct decision** ("完全カバーを選んでください," full coverage), not a Mechanical Lead default — after the hazard radius it is measured against (`rotating_env_max_r`) had grown from 126.424mm to 176.259mm as a side effect of the Rev 5 PCB resize (the PCB bay is part of the rotating assembly), degrading the prior 115.0mm guard's own coverage from ≈77.7% to ≈35.0%. **Not silently carried forward at any point**: MISS-023 was re-opened (OPEN/HIGH, was ACCEPTED-RISK) pending fresh human review, then RESOLVED once that review's own decision was implemented and re-verified (100% coverage, 0.0mm residual gap, re-confirmed via direct boolean CSG non-overlap with the complete rotating envelope) — see `validation/open-issues.md` MISS-023. **Also fixed this session (MISS-036, RESOLVED, unrelated to the PCB resize)**: a real, pre-existing print_layout quadrant-cutting geometry bug that clipped ~35.4% of material off each quadrant (a chord-based cutting tool too small for a 90° wedge) — this STL now has the CORRECT, full-intended geometry, re-confirmed to still hold at the new, much larger radius (4×quadrant volume matches the full-ring volume to 0.0000015%). Still split into 4 quadrants deliberately, **not** because any printer-bed-size limit is known for this project (none is documented anywhere in this repo — this was not invented), **but the comment claiming this "prints on virtually any consumer FDM printer" no longer holds at the new 176.3×176.3mm per-piece bounding box** — corrected (not silently left stale) per MISS-048 (RESOLVED, `validation/open-issues.md`); some smaller consumer print beds (~180mm-class) may not fit this piece, though many larger ones (~220-250mm-class) still do. |
...
| `bench-imu-01-pinch-guard-quadrant.stl` | 176.3 × 176.3 × 14.9 (was 115.0 × 115.0 × 14.9) | **GROWN this session, MISS-023 full closure** (human Chief Engineer direct decision, "完全カバーを選んでください" — see `validation/open-issues.md` MISS-023's resolved entry). `pinch_guard_or` 115.0mm → 176.3mm, exceeding the confirmed 176.259mm `rotating_env_max_r` by construction — 100% hazard-band coverage, 0.0mm residual gap (was 77.7%/11.4mm, then 35.0%/61.3mm post-Rev-5-resize). Internal volume per quadrant: 320,684.98mm³ (4× = 1,282,739.92mm³, matching the independently-re-exported full-ring's own 1,282,739.90mm³ to 0.0000015% — confirming MISS-036's own quadrant-cut fix still correctly reconstructs the full ring's own material budget at this much larger radius). 4×320,684.98mm³×1.27g/cm³(PETG) = **1629.080g** (was 570.56g) — see `validation/open-issues.md` MISS-023/MISS-047/MISS-048 for the full disclosure, including the now-larger per-piece footprint's own printability consequence (MISS-048) |
