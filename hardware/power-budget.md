...
| 12V-class motor rail *(Rev 3, Option A approved — dedicated 2nd input)* | **Approved architecture: separate dedicated motor-power input (e.g. barrel jack), independent of the existing USB 5V logic input** — human Chief Engineer decision, `hardware/power-architecture.md` Decision table, 2026-08-31. Specific connector/any converter part still to be sourced by Component Engineer during Circuit Design | **≈12 V class required for recommended pairing** — driver VM is 8–28 V, so the chosen source must clear 8 V minimum with real margin; 3S-equivalent operation is the intended fit for the recommended motor+driver pair | Rail should be sized against at least the motor's ≈1.05 A target draw and the driver's 2 A continuous / 3 A peak capability; final numeric capability depends on the specific external supply/adapter Circuit Engineer specifies this input for | DS-MTR-017, DS-MTR-020, DS-MTR-032, DS-MTR-034 |
