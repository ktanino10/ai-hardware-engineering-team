...
## Renumbering note (Hardware Lead, 2026-09-11, main-branch merge)

**DS-MCU-068 (above) was renumbered to DS-MCU-073** during the merge of
`origin/main` (PR #9) into this branch, which independently surfaced a
*more complete* version of this same defect class (its own ISS-014,
adopted here as **ISS-027**, additionally correcting VDD/VDDA/VBAT/VSS/
NRST pin-identity errors this record's own 2026-09-09 fix never caught)
under evidence IDs DS-MCU-064 through DS-MCU-067. Since those numbers
were already in use for this record's own DS-MCU-064/065/066 (Rev 3
motor-pin-allocation citations) and DS-MCU-067 (PA9 GPIO-reset-state
citation), this record's colliding IDs were shifted: DS-MCU-064→069,
065→070, 066→071, 067→072, **068→073** — see `validation/change-log.md`
ECO-014 for the full renumbering map. The correction note above is left
as originally written (historical accuracy), with this note as the
live pointer to the current citation.
